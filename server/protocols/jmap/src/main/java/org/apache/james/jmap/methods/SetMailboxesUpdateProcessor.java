...
public class SetMailboxesUpdateProcessor<Id extends MailboxId> implements SetMailboxesProcessor<Id> {
    ...
    public SetMailboxesResponse process(SetMailboxesRequest request, MailboxSession mailboxSession) {
        ...
        request.getUpdate()
            .entrySet()
            .stream()
            .forEach(x -> handleUpdate(x, responseBuilder));
        return responseBuilder.build();
    }

    private void handleUpdate(Entry<String, MailboxUpdateRequest> request, Builder responseBuilder) {
        responseBuilder.updated(request.getKey());
    }
